...
object CSBackground {
    ...
    fun background(
        delay: Long = 0, @WorkerThread function: () -> Unit): ScheduledFuture<*> =
    ...
    fun background(
        delay: Int = 0, @WorkerThread function: () -> Unit): ScheduledFuture<*> =
        executor.background(delay = delay.toLong(), function = function)

    fun backgroundRepeat(
        delay: Long, period: Long, @WorkerThread function: () -> Unit): ScheduledFuture<*> =
        executor.backgroundRepeat(delay = delay, period = period, function = function)

    fun backgroundRepeat(
        period: Long, @WorkerThread function: () -> Unit): ScheduledFuture<*> =
